...
\begin{abstract}
Federated learning represents a paradigm shift in
...
approaches \cite{sheller_federated_2020}. Federated learning emerges as a transformative solution, enabling multiple healthcare institutions to
...
rather than centralising data for analysis \cite{rieke_future_2020}. In this distributed
...
averaging based on the size of their local datasets \cite{xu_federated_2021}. This approach ensures that institutions with larger patient populations
...
and the need for model interpretability in clinical decisionmaking \cite{suk_privacy-preserving_2019}. Medical data often exhibits significant non-IID
...
diagnosis \cite{suk_privacy-preserving_2019,chen_fedhealth_2020,rieke_future_2020}. The ability to train deep learning models on
...
regulatory bodies \cite{sheller_federated_2020,xu_federated_2021}. By enabling secure analysis of
...
expensive in terms of bandwidth \cite{rieke_future_2020}.
...
patient safety and maintain trust \cite{xu_federated_2021}. Future research must
...
\label{appendix:ref-management}

Figure~\ref{fig:zotero} shows a screenshot of the Zotero desktop app, the
reference management system used in this paper, where one
can see:
\begin{itemize}
    \item The “Tutorial Exercises” collection selected in the top-left panel.
    \item All the collection’s references in the centre panel, with one about Federated Learning selected.
    \item The reference’s metadata in the right panel.
    \item The collection’s tags in the bottom-left panel.
\end{itemize}

\begin{figure*}[ht]
    \centering
    \includegraphics[width=\textwidth]{ss1.png}
    \caption{ Zotero reference library for federated learning in healthcare related work overview an some extra sources on evolutionary computation applied to
    machine learning.}
    \label{fig:zotero}
\end{figure*}
